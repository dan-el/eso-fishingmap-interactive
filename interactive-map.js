...
/* Hide all elements of a certain 
 * class
 */
...
/* Toggle all elements of a certain
 * class */
...
/* Set/Unset all elements of classname
 * active via css class
 */
...
/* Unset all elements of .classname active
 * via css class
 */
...
function countFishingHoles(zone,type) {
  var loc = getLocation(zone); // sanitize zone
  if (type == 'oily') { var w = 'foul'; } else { var w = type; } // oily is equal to foul in terms of button id
...
function generateInfoText(alliance,zone) {
  ...
  if (fish) {
	...
	if (innertxt) { txt = txt + innertxt; }
	else { txt = txt + '<em>No rare fish here :-(</em>'; }
	txt = txt + '</div></div>';
  }
